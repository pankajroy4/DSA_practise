...
void insert(struct Que *, struct BST*);
struct BST* del(struct Que*);
void lavel_order_traversal(struct BST *);
...
void insert(struct Que *p, struct BST *x){
    if (p->rear == 9){
        printf("\nQue overflow.");
        return;
    }
...
void lavel_order_traversal(struct BST *pr){
    struct Que Q;
    struct BST *temp;
    Q.rear=-1;
    Q.front=0;
